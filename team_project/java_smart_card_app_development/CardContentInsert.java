...
public class CardContentInsert implements iCardConstant {
	 ...
	 *            수신 날짜
	 */
	public CardData insert(final String cardNumber, final String body, Location location,
						   final long date) {
		if ((cardNumber.equals(KB_CARD_NUMBER) || cardNumber
				.equals(TEST_CARD_NUMBER)) && body.contains("사용")) { // 국민카드
			String[] arrCardMessage = body.split("\n"); // 내용 자르기
			value.clear();
			CardData data = null;
			int buy = 1;
			// 승인 혹은 승인 취소
			...
			try {
				...
				value.put("company", KB_CARD); // 국민 카드 회사
				Log.i(MyActivity.DEBUG_TAG, "long date->" + date);
				value.put("date", arrCardMessage[4]); // 날짜
				value.put("buy", buy); // 승인여부
				value.put("price", arrCardMessage[5]); // 사용금액
				value.put("shop", arrCardMessage[6].replace("사용", "").trim()); // 사용처
				...
				data.setDate(arrCardMessage[4]);
				data.setPrice(arrCardMessage[5]);
				data.setShop(arrCardMessage[6].replace("사용", "").trim());
				if(location != null) {
		...
		} else if (cardNumber.equals(NB_CARD_NUMBER)) { // 농협카드
			String[] arr = body.split("\n"); // 내용 자르기
			value.clear();
			CardData data = null;
			int buy = 1;
			...
			try {
				value.put("no", date); // 날짜 밀리세컨드를 인덱스값으로
				value.put("company", NB_CARD); // 카드 회사
				Log.i(MyActivity.DEBUG_TAG, "long date->" + date);
				value.put("date", arr[4] + " " + arr[2]); // 날짜
				value.put("buy", buy); // 승인여부
				value.put("price", arr[1].split("원")[0]); // 사용금액
				value.put("shop", arr[5]); // 사용처
				if(location != null){
					value.put("lat", ""+location.getLatitude());
					value.put("lng", ""+location.getLongitude());
				...
				data = new CardData();
				data.setMillis(date);
				data.setCardCompany(NB_CARD_NUMBER);
				data.setDate(arr[4]);
				data.setPrice(arr[1].split("원")[0]);
				data.setShop(arr[5]);
				data.setLat(""+location.getLatitude());
				data.setLng(""+location.getLongitude());
			} catch (IndexOutOfBoundsException iobe) {

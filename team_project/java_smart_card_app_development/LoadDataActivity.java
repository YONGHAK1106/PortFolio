...
public class LoadDataActivity extends MyActivity {
	...
	//	}
		...
		if (cursor != null && cursor.moveToFirst()) {
			do {
				...
				data = cardInsert.insert(address, body, null, date);
			...
			// ui 쓰레드로
			LoadDataActivity.this.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					// TODO Auto-generated method stub
					if(mCount >	0){		// 1건 이상일때 토스트
						Toast.makeText(LoadDataActivity.this,
	...
	private class AsyncTaskFileUpload extends
		...
		protected void onPostExecute(final Integer count) { // 전송 완료후
			...
			if (count == insertData.size()) { // 파일 전송이 정상이면
				if (count > 0) {
					Toast.makeText(LoadDataActivity.this, "모든 내역이 전송되었습니다.",
							Toast.LENGTH_LONG).show();
				} else {
					Toast.makeText(LoadDataActivity.this, "전송 내역이 없습니다.",
							Toast.LENGTH_LONG).show();
				}
			} else {
				Toast.makeText(
			...
			getCardDatase();
			Intent intent = new Intent(LoadDataActivity.this, MainTabsActivity.class);
